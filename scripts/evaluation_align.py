...
if __name__ == "__main__":
    ...
    # Scene
    order =  ['1','2','3','4', '' ,'5','5d','5n','','6','6d','6n','','7','7d','7n','','8','8d','8n']
    fig, ax = plt.subplots()
    df_long = pd.melt(df, id_vars=['scene'], value_vars=[
                      'translation [mm]', 'rotation [degrees]'], var_name=['user'])
    ax = sns.boxplot(data=df_long, x='scene', y='value', hue='user', fliersize=1, palette="Blues", width=0.8,
                     medianprops=dict(color="red", alpha=0.7), linewidth=0.7, whis=100., autorange='auto',order=order)
    ...
    df['scene'] = np.where(np.isin(df['scene'],groundtruth_scenes),'synthetic', df['scene'])
    df['scene'] = np.where(np.isin(df['scene'],manual_scenes),'manual', df['scene'])
    df['scene'] = np.where(np.isin(df['scene'],sensor_scenes),'depth', df['scene'])
    df['scene'] = np.where(np.isin(df['scene'],nerf_scenes),'nerf', df['scene'])

    fig, ax = plt.subplots()
    df_long = pd.melt(df, id_vars=['scene'], value_vars=[
                      'translation [mm]', 'rotation [degrees]'], var_name=['user'])
    ax = sns.boxplot(data=df_long, x='scene', y='value', hue='user', fliersize=1, palette="Blues", width=0.4, 
                     medianprops=dict(color="red", alpha=0.7), linewidth=0.8, autorange=auto)
