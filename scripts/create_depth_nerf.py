...
if __name__ == "__main__":
    ...
    if(os.path.exists(args.dataset)):
        ...
        if not "Nerf" in config:
            print(f"Configuration file {args.dataset} does not contain Nerf parameter.")
    ...
    if config["view"] not in [180, 360]:
        ...
        print(f"Error: The config paramter view for degrees of camera views {view_value} is invalid. Only 180 or 360 allowed.")
        os.sys.exit(1)
    ...
    for scene_id in scenes:
        ...
        scene_dir = os.path.join(scene_file_reader.root_dir, scene_file_reader.scenes_dir, scene_id)
        ...
        with open(nerf_intrinsic_file,'w') as fp:
           fp.write("%d %d %.12f %.12f %.12f %.12f %.12f %.12f %.12f %.12f %.12f %d\n" % (
               intrinsic.width, intrinsic.height, intrinsic.fx, 0.0, intrinsic.cx, 0.0, intrinsic.fy, intrinsic.cy, 0.0, 0.0, 1.0, config["view"]
           ))
           fp.flush()
